...
if __name__ == '__main__':
        ...
        if 'version' in version:
            versions[version['version']] = '{}\t{}\t{}'.format(version['id'], 
                        time.strftime('%Y-%m-%d:%H:%M:%S', 
                                time.localtime(version['versionTime']/1000)), version['versionComment'])
    
    #print(json.dumps(versions,indent=2, sort_keys=True))

    print(" Ver | {:37}\t{:20}\tcomment".format(' templateId for version of template', 'Time of commit'))
    ...
    print()
    print("Getting template for latest version: {}".format(latest_version))
